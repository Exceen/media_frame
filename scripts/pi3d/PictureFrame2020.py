...
def truncate_to_approximate_arial_width(txt, width):
    tmp_txt = __truncate_to_approximate_arial_width(txt, width)
    if tmp_txt != txt:
      tmp_txt = tmp_txt.strip()
      txt = tmp_txt[:len(tmp_txt)-2].strip() + '...'
    return txt

def __truncate_to_approximate_arial_width(st, width):
    size = 0 # 1000 = 1 inch
    ...
    def on_message(client, userdata, message):
      ...
      if message.topic == "frame/date_from": # NB entered as mqtt string "2016:12:25"
        ...
        refresh = True
          ...
          next_pic_num -= 1
          refresh = True
      elif message.topic == "frame/music_on":
          config.SHOW_TEXT = 8
          next_pic_num -= 1
          refresh = True
...
while DISPLAY.loop_running():
    ...
    if nFi > 0:
      ...
      if config.SHOW_TEXT > 0 or paused: #was SHOW_TEXT_TM > 0.0
        ...
        if config.LOAD_GEOLOC and (config.SHOW_TEXT & 4) == 4: # location
          ...
          gap = " "
        if (config.SHOW_TEXT & 8) == 8: # music
          txt = 'No song information'
          try:
            with open(config.CURR_TRACK_PATH, 'r') as f:
              txt = f.read().replace('\n', ' ').strip()
          except Exception as e:
            pass
        if paused:
        ...
        # txt = "this is a test string and it is very long hello world we are there and are here - in 2021 and i added this text to test the text length on two lines and this is a test string asdf jkl awwer"
        if config.TWO_LINE_TRACK:
          if ' - ' in txt:
            txt_artist, txt = txt.split(' - ', 1)
          else:
            txt_artist, txt = '-', txt
        ...
        if config.TWO_LINE_TRACK:
          textblock_artist.set_text(text_format="{}".format(" "))
          textblock_artist.colouring.set_colour(alpha=0.0)
  ...
  if nFi <= 0:
    ...
    if config.TWO_LINE_TRACK:
      textblock_artist.set_text("")
      textblock_artist.colouring.set_colour(alpha=1.0)
    next_tm = tm + 1.0
    ...
    if config.TWO_LINE_TRACK:
      textblock_artist.colouring.set_colour(alpha=alpha)

    text.regen()
    if config.TWO_LINE_TRACK:
      text_artist.regen()
    text_bkg.set_alpha(alpha * 0.85)
    if len(textblock.text_format.strip()) > 0: #only draw background if text there
      text_bkg.draw()
    if config.TWO_LINE_TRACK:
      if len(textblock_artist.text_format.strip()) > 0: #only draw background if text there
        text_bkg.draw()
